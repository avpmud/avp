...
#define IMCPERM(ch)           (CH_IMCDATA((ch))->m_Perm)
#define IMCFLAG(ch)           (CH_IMCDATA((ch))->m_Flags)
//#define FIRST_IMCIGNORE(ch)   (CH_IMCDATA((ch))->imcfirst_ignore)
//#define LAST_IMCIGNORE(ch)    (CH_IMCDATA((ch))->imclast_ignore)
#define IMC_IGNORES(ch)		  (CH_IMCDATA((ch))->m_Ignores)
#define IMC_LISTEN(ch)        (CH_IMCDATA((ch))->m_Listen)
#define IMC_DENY(ch)          (CH_IMCDATA((ch))->m_Denied)
#define IMC_RREPLY(ch)        (CH_IMCDATA((ch))->m_RReply)
#define IMC_RREPLY_NAME(ch)   (CH_IMCDATA((ch))->m_RReplyName)
...
#define IMC_COMMENT(ch)       (CH_IMCDATA((ch))->m_Comment)
//#define IMCTELLHISTORY(ch,x)  (CH_IMCDATA((ch))->imc_tellhistory[(x)])
#define IMC_TELLHISTORY(ch)   (CH_IMCDATA((ch))->m_TellHistory)
...
typedef /*struct imcCharData*/ struct PlayerIMC2Data IMC_CHARDATA; /* Player flags */
...
typedef void IMC_FUN( CharData * ch, char *argument );
#define IMC_CMD( name ) void (name)( CharData *ch, char *argument )
...
void imc_load_pfile( CharData * ch, char *tag, int num, char *line );
void imc_save_pfile( struct CharData *ch, FBFILE * fp );
#endif
