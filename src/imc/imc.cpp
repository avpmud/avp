...
void imc_printf( CharData * ch, const char *fmt, ... ) __attribute__ ( ( format( printf, 2, 3 ) ) );
void imcpager_printf( CharData * ch, const char *fmt, ... ) __attribute__ ( ( format( printf, 2, 3 ) ) );
char *imc_funcname( IMC_FUN * func );
IMC_FUN *imc_function( const char *func );
char *imc_send_social( CharData * ch, char *argument, int telloption );
...
IMC_CHANNEL *first_imc_channel = NULL;
IMC_CHANNEL *last_imc_channel = NULL;
REMOTEINFO *first_rinfo = NULL;
REMOTEINFO *last_rinfo = NULL;
IMC_BAN *first_imc_ban = NULL;
IMC_BAN *last_imc_ban = NULL;
IMCUCACHE_DATA *first_imcucache = NULL;
IMCUCACHE_DATA *last_imcucache = NULL;
IMC_COLOR *first_imc_color = NULL;
IMC_COLOR *last_imc_color = NULL;
IMC_CMD_DATA *first_imc_command = NULL;
IMC_CMD_DATA *last_imc_command = NULL;
IMC_HELP_DATA *first_imc_help = NULL;
IMC_HELP_DATA *last_imc_help = NULL;
IMC_PHANDLER *first_phandler = NULL;
IMC_PHANDLER *last_phandler = NULL;
...
char *imcone_argument( const char *argument, char *arg_first );
char *imc_strip_colors( const char *txt );  
char *color_itom( const char *txt, CharData * ch );  
char *color_mtoi( const char *txt );  
void imc_to_char( const char *txt, CharData * ch );  
void imc_to_pager( const char *txt, CharData * ch );  
...
inline bool imc_hasname( const Lexi::String &list, const char *member ) { return imc_hasname(list.c_str(), member); }
void imc_addname( Lexi::String &list, char *member );  
void imc_removename( Lexi::String &list, char *member );  
...
CharData *imc_find_user( char *name );  
char *imcgetname( char *from );  
bool imc_isbanned( char *who );  
bool imc_isignoring( CharData * ch, const char *ignore );  
...
bool check_mud( CharData * ch, char *mud );  
bool check_mudof( CharData * ch, const char *mud );  
int get_imcpermvalue( const char *flag );  
bool imccheck_permissions( CharData * ch, int checkvalue, int targetvalue, bool enforceequal );  
...
void imcformat_channel( CharData * ch, IMC_CHANNEL * d, int format, bool all );  
...
void imc_update_tellhistory( CharData * ch, const char *msg );  
...
void imc_sendmessage( IMC_CHANNEL * c, const char *name, char *text, int emote );  
PFUN( imc_recv_chanwhoreply );  
char *get_local_chanwho( IMC_CHANNEL *c );  
PFUN( imc_recv_chanwho );  
void imc_sendnotify( CharData * ch, char *chan, bool chon );  
PFUN( imc_recv_channelnotify );  
char *imccenterline( const char *string, int length );  
char *imcrankbuffer( CharData * ch );  
void imc_send_whoreply( const char *to, char *txt );  
void imc_send_who( const char *from, char *to, char *type );  
...
//   snprintf( buf, LGST * 2, "%s\033[0m", color_itom( txt, ch ) );
   snprintf( buf, LGST * 2, "%s`n", color_itom( txt, ch ) );
   ch->Send( "%s", buf );
   return;
}

/* Modified version of Smaug's ch_printf_color function */
void imc_printf( CharData * ch, const char *fmt, ... )
{
   char buf[LGST];
   va_list args;
...
void imcpager_printf( CharData * ch, const char *fmt, ... )
{
   char buf[LGST];
   va_list args;
...
void imc_addname( Lexi::String &list, char *member )
{
   ...
   if( list.empty() )
      ...
      snprintf( newlist, LGST, "%s %s", list.c_str(), member );
...
void imc_removename( Lexi::String &list, char *member )
{
   ...
   imcstrlcpy( newlist, imcstrrep( list.c_str(), member, "" ), LGST );
...
/*
 * Returns a CharData class which matches the string
 */
CharData *imc_find_user( char *name )
{
   CharData *vch = NULL;
   ...
   FOREACH(DescriptorList, descriptor_list, iter)
   {
      ...
      if( ( vch = d->m_Character ? d->m_Character : d->m_OrigCharacter ) != NULL && !strcasecmp( CH_IMCNAME( vch ), name )
          && d->GetState()->IsPlaying() )
         return vch;
   }
   return NULL;
}
...
//   IMC_IGNORE *temp;
    ...
    */
//   for(temp = FIRST_IMCIGNORE(ch); temp != LAST_IMCIGNORE(ch); temp = temp->next)
//      if( !str_prefix( temp->name, ignore ) )
//         return TRUE;

	FOREACH(Lexi::StringList, IMC_IGNORES(ch), t)
		if ( !str_prefix( t->c_str(), ignore ))
			return TRUE;
   return FALSE;
}
...
void imcformat_channel( CharData * ch, IMC_CHANNEL * d, int format, bool all )
{
   ...
   if( all )
   {
      for( c = first_imc_channel; c; c = c->next )
      {
         ...
         {
            snprintf( buf, LGST, "~y[~g%s~y] ~!%%s~y: %%s", c->local_name );
            IMCSTRFREE( c->regformat );
         ...
         {
            snprintf( buf, LGST, "~y[~g%s~y] ~!%%s", c->local_name );
            IMCSTRFREE( c->socformat );
   ...
   else
   {
      ...
      if( format == 1 || format == 4 )
      {
         snprintf( buf, LGST, "~y[~g%s~y] ~!%%s~y: %%s", d->local_name );
      ...
      if( format == 3 || format == 4 )
      {
         snprintf( buf, LGST, "~y[~g%s~y] ~!%%s", d->local_name );
...
*/
	if (IMC_TELLHISTORY(ch).size() == MAX_IMCTELLHISTORY)
		IMC_TELLHISTORY(ch).pop_front();
	IMC_TELLHISTORY(ch).push_back(new_msg);
...
//   for( d = first_descriptor; d; d = d->next )
   ...
   {
      if( (*d)->GetState()->IsPlaying() && ( ch = (*d)->GetOriginalCharacter() ) != NULL
          && IMCPERM( ch ) >= level )
...
char *imc_assemble_who( void )
{
   ...
   FOREACH(DescriptorList, descriptor_list, d)
   {
      ...
      if( person && (*d)->GetState()->IsPlaying())
      {
...
void imc_process_who( const char *from )
{
...
void imc_process_finger( char *from, char *type )
{
   ...
   snprintf( buf, IMC_BUFF_SIZE,
             ...
             ( !IMC_LISTEN( victim ).empty() ) ? IMC_LISTEN( victim ).c_str() : "None" );
...
void imc_send_whois( const char *from, char *user )
{
   IMC_PACKET *p;
...
void imc_send_beep( const char *from, char *to )
{
   IMC_PACKET *p;
   ...
   while( nleft > 0 )
   {
      if( ( nwritten = send( this_imcmud->desc, ptr, nleft, 0 ) ) <= 0 )
      {
		...
		int iErr = errno;
         ...
         if( nwritten < 0 )
            imclog( "Write error on socket: %s", strerror( iErr ) );
         else
...
void imc_loop( void )
{
   ...
   if( !this_imcmud || this_imcmud->state == IMC_OFFLINE || this_imcmud->desc == -1 )
      return;
...
void imc_adjust_perms( CharData * ch )
{
   if( !this_imcmud )
      return;
...
void imc_char_login( CharData * ch )
{
   char buf[SMST];
   int gender, sex;
...
bool imc_loadchar( CharData * ch, Lexi::Parser &parser )
{
   if( IS_NPC( ch ) )
      return FALSE;
   ...
   IMCPERM(ch) = parser.GetInteger("Permission");
   IMCFLAG(ch) = parser.GetInteger("Flags");
   IMC_COMMENT(ch) = parser.GetString("Comment");
   IMC_LISTEN(ch) = parser.GetString("Listen");
   
   
   if( !IMC_LISTEN( ch ).empty() && this_imcmud && this_imcmud->state == IMC_ONLINE )
    {
       IMC_CHANNEL *channel = NULL;
       const char *channels = IMC_LISTEN( ch ).c_str();
       char arg[SMST];
   ...
   IMC_DENY( ch ) = parser.GetString("Deny");
    if( !IMC_DENY( ch ).empty() && this_imcmud && this_imcmud->state == IMC_ONLINE )
    {
       IMC_CHANNEL *channel = NULL;
       const char *channels = IMC_DENY( ch ).c_str();
       char arg[SMST];
	...
	int numIgnores = parser.NumFields("Ignores");
	for (int i = 0; i < numIgnores; ++i)
		IMC_IGNORES(ch).push_back(parser.GetIndexedString("Ignores", i));

	return true;
}

void imc_savechar( CharData * ch, Lexi::OutputParserFile &file )
{
   if( IS_NPC( ch ) )
      return;
   
   file.WriteInteger("Permission", IMCPERM(ch));
   file.WriteInteger("Flags", IMCFLAG(ch));
   file.WritePossiblyLongString("Comment", IMC_COMMENT(ch));
   file.WriteString("Listen",IMC_LISTEN(ch));
   file.WriteString("Deny", IMC_DENY(ch));
   file.BeginSection("Ignores");
   {
       int count = 0;
	   FOREACH(Lexi::StringList, IMC_IGNORES(ch), i)
	   	file.WriteString(Lexi::Format("Ignore %d", ++count).c_str(), *i);
   }
   file.EndSection();
}
      ...
      switch ( word[0] )
      {
         ...
         case 'T':
            if( !strcasecmp( word, "Text" ) )
            {
               ...
               while( ( hbuf[num] = fgetc( fp ) ) != EOF && hbuf[num] != (char)0xA2 && num < ( LGST - 2 ) )
                  num++;
...
int ipv4_connect( void )
{
   ...
   if( connect( desc, ( struct sockaddr * )&sa, sizeof( sa ) ) == -1 )
   {
      ...
      if( iErr != EINPROGRESS )
      {
         imclog( "%s: Failed connect: Error %d: %s", __FUNCTION__, iErr, strerror( iErr ) );
         perror( "connect" );
         close( desc );
         return -1;
      }
...
*/
	FOREACH(Lexi::StringList, IMC_TELLHISTORY(ch), s)
		imc_to_char(s->c_str(), ch);
...
IMC_CMD( imcreply )
{
   ...
   if( !check_mudof( ch, IMC_RREPLY( ch ).c_str() ) )
      return;
   ...
   if( argument[0] == '@' )
   {
      ...
      imc_send_tell( CH_IMCNAME( ch ), IMC_RREPLY( ch ).c_str(), p, 2 );
      p2 = imc_send_social( ch, buf2, 2 );
      if( !p2 || p2[0] == '\0' )
         return;
      snprintf( buf1, LGST, "~WImctell ~C%s: ~c%s\n", IMC_RREPLY( ch ).c_str(), p2 );
   }
   else
#endif
   if( argument[0] == ',' )
   {
      ...
      while( isspace( *argument ) )
         argument++;
      imc_send_tell( CH_IMCNAME( ch ), IMC_RREPLY( ch ).c_str(), color_mtoi( argument ), 1 );
      snprintf( buf1, LGST, "~WImctell: ~c%s %s\n", IMC_RREPLY( ch ).c_str(), argument );
   }
   else
   {
      imc_send_tell( CH_IMCNAME( ch ), IMC_RREPLY( ch ).c_str(), color_mtoi( argument ), 0 );
      snprintf( buf1, LGST, "~cYou imctell ~C%s ~c'~W%s~c'\n", IMC_RREPLY( ch ).c_str(), argument );
   }
   imc_to_char( buf1, ch );
   imc_update_tellhistory( ch, buf1 );
...
IMC_CMD( imcignore )
{
   ...
   if( !arg || arg[0] == '\0' )
   {
      ...
      FOREACH(Lexi::StringList, IMC_IGNORES(ch), ign)
         imc_printf( ch, "%s\n", ign->c_str());

      if( IMC_IGNORES(ch).empty() )
         imc_to_char( " none\n", ch );
      else
         imc_printf( ch, "\n[total %d]\n", IMC_IGNORES(ch).size() );
   ...
   {
      IMC_IGNORES(ch).push_back(argument);
...
IMC_CMD( imcpermset )
{
   ...
   {
      IMC_CHANNEL *channel = NULL;
      const char *channels = IMC_LISTEN( victim ).c_str();
...
char *imc_find_social( CharData * ch, char *sname, char *person, char *mud, int victim )
{
#if 0
...
char *imc_act_string( const char *format, CharData * ch, CharData * vic )
{
...
#if 0
CharData *imc_make_skeleton( char *name )
{
   CharData *skeleton;

   IMCCREATE( skeleton, CharData, 1 );
...
void imc_purge_skeleton( CharData * skeleton )
{
   if( !skeleton )
      return;
...
bool imc_command_hook( CharData * ch, char *command, char *argument )
{
   IMC_CMD_DATA *cmd;
   IMC_ALIAS *alias;
   IMC_CHANNEL *c;
   ...
   if( !imc_hasname( IMC_LISTEN( ch ).c_str(), c->local_name ) )
   {
